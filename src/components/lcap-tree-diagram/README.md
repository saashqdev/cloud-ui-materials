...
# LcapTreeDiagram Tree Structure Diagram
...
| --------- | ---- | ------- | ------- | ----------- |
| data-source | Array\<Item\> \| Function \| object \| DataSource | | | The data source of the table, the dataset object or the logic of returning the dataset |
| value-field | string | | `'id'` | Attribute that identifies the selected value |
| text-field | string | | `'label'` | Field name to display the text in |
| showChildDotNum | boolean | | `true` | Enabled by default; when enabled, displays the total number of child nodes |
| parent-field | string | | `'parentId'` | Attribute used to identify the parent node in the element type of the collection |
| data-entity | string | | `''` | For anonymous data structures, the data entity name needs to be passed, otherwise it is not needed |
...
| ----- | ---- | ----------- |
| $event | ChangeItemEvent | Mouse event object |
