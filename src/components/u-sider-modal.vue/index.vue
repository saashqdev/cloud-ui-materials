<template>
    <div ref="container" v-on="$listeners" :class="[$style.root, $style.container, position === 'left' ? $style.posHideLeft : $style.posHideRight]" :style="showOrHideStyle">
        <div :class="[$style.showButton, position === 'left' ? $style.leftButton : $style.rightButton]" kubevue-click-enabled @click.stop="toggle">{{buttonShowText}}</div>
        <div :class="$style.content" kubevue-slot-name="default" :kubevue-scope-id="$parent.$parent.$options._scopeId">
            <slot></slot>
