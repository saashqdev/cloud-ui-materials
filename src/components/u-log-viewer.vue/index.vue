<template>
    <div :class="$style.root" v-show="visible" :color="currentColor" :display="currentDisplay" :maximized="isMaximized" :position="position">
        <div :class="$style.head">
            <div :class="$style.buttons">
                ...
                <!-- <div :class="$style.button" role="search" :title="'Search'"></div> -->
                <div :class="$style.button" role="clear" title="Clear Log" @click="clear()"></div>
                <div :class="$style.button" v-if="fetchLogs" role="fetchLogs" title="Pull Logs" @click="fetchLogs()"></div>
                <div :class="$style.button" role="brightness"
                    :title="'Switch to' + (currentColor === 'dark' ? 'Light color' : 'Dark color')"
                    @click="toggleColor()"></div>
                    <div :class="$style.button" role="maximize" :title="isMaximized ? 'Exit Full Screen' : 'Full Screen'" @click="toggleMaximize()"></div>
                <div :class="$style.button" v-if="openInNewTab" role="openInNewTab" title="Open in New Tab" @click="_openInNewTab()"></div>
                <div v-if="$options.propsData.hasOwnProperty('visible')" :class="$style.button" role="close" title="Close" @click="close()"></div >
            </div>
        </div>
        ...
        <div :class="$style.foot">
            <div :class="$style.button" role="scrollToTop" title="Scroll to Top" @click="scrollToTop()"></div>
            <div :class="$style.button" role="scrollToBottom" title="Scroll to the Bottom" @click="scrollToBottom()"></div>
        </div>
        <slot></slot>
    </div>
    </template>
    ...
    export default {
            ...
            display: {
                ...
                validator: (value) =>
                    [
                        ...
                        'static', // Displayed in the file stream, 'same as block'
                        'full',
                        'fixed', // 'modal',
                        'fullWindow',
                        'fullScreen',
            ...
            currentDisplay: {
                handler(currentDisplay, oldDisplay) {
                    ...
                    if (currentDisplay === 'fullWindow') {
                        // Dealing with the z-index problem, modifying the z-index of the parent layer is more complicated
                        // There is no more general way except to migrate $el to document.body
        ...
        beforeCreate() {
            // With buffering, different component instances should not be mixed
        ...
        methods: {
            ...
            flush() {
                ...
                this.buffer = []; // If the log is at the bottom, it will continue to roll
